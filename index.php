...
.card{
  ...
  border-radius:28px;
  padding:30px 24px;
  box-shadow:0 18px 50px rgba(20,71,120,.12);
  text-align:center;
}
...
.logo{
  ...
  display:grid;
  place-items:center;
  background:linear-gradient(135deg,var(--primary),#4f9df5);
  color:white;
  font-size:36px;
}
...
<div class="card">
  ...
  <div class="sub">
    สแกนเพื่อตรวจสอบสิทธิ์รับของรางวัลประจำวัน
  </div>
  ...
  <div id="available" class="status ok hidden">
    <div class="big">✅ พร้อมรับของรางวัลวันนี้</div>

    <div>
      กรุณาแสดงหน้านี้ให้เจ้าหน้าที่
    </div>
  ...
  <div id="redeemed" class="status bad hidden">
    <div class="big">❌ ใช้สิทธิ์ของวันนี้แล้ว</div>

    <div>
      อุปกรณ์นี้ได้รับของรางวัลสำหรับวันนี้ไปแล้ว
    </div>
  ...
  <div class="footer">
    1 Browser / 1 สิทธิ์ / วัน • AI Center
  </div>
...
async function checkReward(){
  ...
  try{
    ...
    if(data.status === 'redeemed'){
      ...
      document
        .getElementById('redeemedTime')
        .textContent =
          data.redeemed_at
          ? 'รับเมื่อ ' + data.redeemed_at
          : '';
